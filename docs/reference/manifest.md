...
| `ux.notify` | bool | `true` | notify you when it is YOUR move (a gated write or a CLI prompt) — opt-out. **The desktop notification itself is macOS and Linux only**: mokata raises it with `osascript` (macOS) or `notify-send` (Linux). **Windows ships no visual arm** — a toast would require handing PowerShell a script string, which mokata's notifier forbids, or a third-party dependency. On Windows this setting still gates the *sound* below |
| `ux.notify_audio` | bool | `true` | play a sound with the notification; `false` keeps the notification, silent — opt-out. macOS `afplay`, Linux `canberra-gtk-play`/`paplay` (else the terminal bell), **Windows `winsound.MessageBeep`** — stdlib, so no dependency is added. ⚠ On Windows the sound is **the whole channel** (there is no banner), and a successful call means the sound was *queued with the OS*, not that anything was audible — a muted device or a session with no audio endpoint reports nothing back |
