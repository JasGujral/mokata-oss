...
| `migrate` | Migration plumbing — a **one-time, human-gated** move of a **deprecated** channel (`vault`) into the canonical store, run from the shell. The detect-and-offer **is** in-harness: the once-per-repo deprecation warning a read emits names this command (the `worktree` pattern), so the gap is disclosed, not silent. It retires with the channels at 0.0.18. |
