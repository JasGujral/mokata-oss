...
WHAT COUNTS AS "EXCLUDED": PRESENCE, NOT MEMBERSHIP
---------------------------------------------------
The `--exclude` list holds two different kinds of entry. `docs/build/` and `scripts/release.sh`
exist HERE and will not exist THERE — a test that reads one behaves differently across the
boundary. `.mutate.lock`, `dist/` and `site/` are regenerable artifacts that exist in NEITHER tree
until something builds them — a test naming one already behaves identically on both sides, and
condemning it would be inventing an offender. So the caller narrows the exclude set to the entries
**present in the tree being swept** (`present_excludes`); the boundary only bites where presence
differs. This is derived from the tree, not typed as a second list.
...
def present_excludes(root, excludes):
    """The literal excludes that EXIST in `root` — the ones whose presence differs across the
    boundary. An excluded path absent from both trees cannot make a test behave differently."""
    return frozenset(
        e for e in literal_excludes(excludes)
        if os.path.exists(os.path.join(root, *e.split("/"))))
