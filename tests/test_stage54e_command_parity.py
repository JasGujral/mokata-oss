...
EXEMPT = {"unsetup", "harness", "route", "detect", "validate", "bootstrap",
          ...
          # DG-7 (0.0.16) — `release-notes-check` is the disclosure twin of `release-check`:
          # a pure/offline cut preflight asserting RELEASE_NOTES.md announces the tag and
          # carries every fact CHANGELOG.md declares under `### Known limitations`. Run from
          # the shell by release.sh and by the tag-triggered workflow; there is no in-harness
          # workflow it belongs to, for the same reason `release-check` has none.
          "release-notes-check",
