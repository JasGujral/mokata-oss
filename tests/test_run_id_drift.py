...
class TestRootRecovery(unittest.TestCase):
    ...
    def test_round_trips(self):
        """The roots are spelled in the PLATFORM's canonical form, not as POSIX literals.

        `root_of_state_dir` canonicalises separators on the way back (`normpath`), so a hand-typed
        `/tmp/x` comes back as `\\tmp\\x` on Windows — THE SAME PATH, a different string. The old
        fixture asserted the fixture's own spelling rather than the inverse, and went red on
        Windows against a product that had round-tripped correctly.

        Canonical form is also the only domain the pair has to hold on: every root the product ever
        hands `state_dir` arrives from `abspath`/`realpath`/repo discovery and is already canonical,
        so `normpath` is a no-op on it. The assertion stays EXACT — normalising the input is not
        normalising the comparison, and a genuine break in the inverse still fails here."""
        # `normpath(sep)` is the FILESYSTEM ROOT, and it is here because nothing graded it: it is
        # the only input that reaches `root_of_state_dir`'s `or os.sep` arm, where the trimmed
        # parts are empty and returning them verbatim would hand every caller "" — a store at the
        # root recovering NO repo instead of `/`. Verified in both path dialects ('/' on POSIX,
        # '\\' on Windows), so it costs the round trip nothing to carry.
        for root in (os.path.normpath("/tmp/x"), os.path.normpath("/a/b/c"),
                     os.path.normpath(os.sep), os.path.abspath(".")):
    ...
    def test_a_non_state_dir_recovers_nothing_rather_than_guessing(self):
        # `state_dir("")` is the layout tail with NO root in front of it, spelled by the product
        # rather than typed here. It is the exact-length case — `len(parts) == len(tail)` — and it
        # must recover None: there is no repo above it. The stage-29 mutation run found this arm
        # ungraded (`<=` relaxed to `<` survived), and under that relaxation this input recovers
        # the FILESYSTEM ROOT, so a relative store would resolve every run against `/`.
        for bad in ("/tmp", "/tmp/state", "", "/a/.mokata/state", state_dir("")):
