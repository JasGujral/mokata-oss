...
class TestNoSpawnResolvesThroughSystem32(unittest.TestCase):
    ...
    def test_the_declared_shadow_set_agrees_with_the_REAL_System32(self):
        """⭐ THE DECLARED THRESHOLD, CHECKED WHERE IT CAN BE. `SYSTEM32_SHADOWED` is sourced from
        Microsoft's A-Z Windows commands page — and that page **does not list `bash`**, the very
        name whose presence in System32 cost the 0.0.18 cut twenty failures. So the declared set
        is a lower bound with unknown slack, and a threshold nothing ever checks is no better than
        a corpus nothing ever walks.

        On Windows this asks the filesystem. A declared member that is NOT there is drift worth
        knowing about (the set is over-broad); the reverse — something in System32 the set omits —
        is caught by `shadowed_on_host` being what `is_shadowed` actually consults here."""
        missing = sorted(n for n in WP.SYSTEM32_SHADOWED if not WP.shadowed_on_host(n))
        self.assertEqual([], missing,
                         "declared as System32-shadowed but absent from %s: %r"
                         % (WP.system32_dir(), missing))
