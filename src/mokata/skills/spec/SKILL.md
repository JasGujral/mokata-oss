...
- emit the spec — `spec_emit` / `mokata spec emit` (human-gated): persists the run's spec AND the shared spec corpus in one commit (read it back with `spec_show`)
