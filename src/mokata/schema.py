...
# ---- detect strategies that are GONE but must still PARSE, and why that is not a shim ---------
#
# SIMP.S3 (0.0.18, lane D slice 1) removed the `obsidian` strategy with the backend it detected.
# It stays ACCEPTED by the validator, and the reason is the remedy: `init_repo` wrote
# `tools.obsidian.detect.type = "obsidian"` into every manifest generated by the `full`/`custom`
# profiles at 0.0.17, so rejecting it makes those manifests INVALID — and `Surface.load` is the
# first thing every command does, INCLUDING `mokata config set memory_store sqlite`, which is the
# fix the removal notice tells the user to run. A repo that cannot load cannot be repaired, and
# "your manifest is invalid: 'obsidian' is not one of [...]" tells a user nothing about the notes
# on their disk. Loud belongs at the memory-resolution path, where it can say what happened to
# the DATA (`memory.selection._refuse_removed_memory_chain`), not at the parser.
#
# ⚠ THIS IS NOT A COMPATIBILITY BRANCH (doc 85 §7d). Nothing READS the strategy — `Detector`
# has no arm for it, so it resolves to "absent" like any unknown type, and there is no second
# code path anywhere. It is one declared tuple that keeps a user's own file parseable, which is
# §7d's ONE EXCEPTION stated in the smallest possible way.
            ...
            "additionalProperties": {
                ...
                "properties": {
                    ...
                    "detect": {
                        ...
                        "required": ["type"],
                        "properties": {
                            "type": {"enum": list(PARSEABLE_DETECT_TYPES)},
                            "name": {"type": "string"},
                        },
                    },
                },
