    ...
    return [
        ...
        CommandSurface("spec", slash=("spec",), mcp_read=("spec_show",),
                       ...
                       note="SI-DEV.0 — EMIT the spec: the completeness gate, then a human-gated "
                            ...
                            "(`mokata spec emit --file`) is the use-anywhere surface and — row A3 "
                            "— drives the SAME propose path when it has no human to ask: at "
                            "a TTY it prompts and an explicit No ends there, off one it stages a "
                            "proposal and exits 3, so the two surfaces no longer disagree about "
                            "what a refusal leaves behind. This is the "
